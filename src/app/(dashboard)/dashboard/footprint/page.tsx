...
export default async function FootprintPage() {
  ...
  const chartData = footprint?.by_model?.map((m: { model: string; carbon_kg: number }) => ({
    model: m.model,
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold text-white mt-1">{Math.round(report.carbon_kg)} <span className="text-lg text-gray-400">kg</span></p>
          <p className="text-gray-500 text-xs mt-1">CO2 equivalent this period</p>
        </div>
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-4">
          <p className="text-gray-400 text-sm">Total Water</p>
          <p className="text-3xl font-bold text-white mt-1">{Math.round(report.water_liters / 1000)}k <span className="text-lg text-gray-400">L</span></p>
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold text-green-400 mt-1">
            {footprint?.savings_potential_kg ? `${Math.round(footprint.savings_potential_kg)} kg` : '—'}
          </p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-700">
                  ...
                  <th className="text-right text-gray-400 font-medium px-4 py-3">Water (L)</th>
                  <th className="text-right text-gray-400 font-medium px-4 py-3">Energy (Wh)</th>
                </tr>
              </thead>
              <tbody>
                {footprint.by_model.map((m: { model: string; carbon_kg: number; water_liters: number; energy_wh: number }, i: number) => (
                  <tr key={i} className="border-b border-gray-700 last:border-0">
                    <td className="px-4 py-3 text-white font-mono text-xs">{m.model}</td>
                    <td className="px-4 py-3 text-gray-300 text-right">{m.carbon_kg?.toFixed(3)}</td>
                    <td className="px-4 py-3 text-gray-300 text-right">{m.water_liters?.toFixed(1)}</td>
                    <td className="px-4 py-3 text-gray-300 text-right">{m.energy_wh?.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-xl p-4">
        ...
        <p className="text-gray-500 text-xs leading-relaxed">
          Carbon calculated using model-specific energy intensity (ArXiv 2505.09598), regional grid carbon
          intensity (EPA eGRID 2024 / IEA 2024), and a PUE of 1.1 for hyperscale data centres. Water consumption
          uses a WUE of 1.9 L/kWh (The Green Grid benchmark) with regional stress multipliers from the
